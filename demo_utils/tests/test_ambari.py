...
class TestAmbariClient(unittest.TestCase):
	...
	@mock.patch('demo_utils.shell.Shell.run', side_effect=[[yarn_service_res, ''], [missing_service_res, '']])
	...
	@mock.patch('demo_utils.shell.Shell.run', return_value=[json.loads(service_info_stopped), ''])
	@mock.patch('demo_utils.ambari.Ambari.get_service', return_value=[json.loads(service_info_stopped), ''])
	@mock.patch('demo_utils.curl_client.CurlClient.make_request', return_value=[change_response_diff, ''])
	...
	@mock.patch('demo_utils.shell.Shell.run', side_effect=[[service_info_starting, ''], [service_info_starting, ''], [service_info_started, '']])
	@mock.patch('demo_utils.ambari.Ambari.get_service', return_value=[json.loads(service_info_stopped), ''])
	@mock.patch('demo_utils.curl_client.CurlClient.make_request', return_value=[change_response_diff, ''])
	...
	@mock.patch('demo_utils.shell.Shell.run', side_effect=[[service_info_starting, ''], [service_info_starting, ''], [service_info_started, '']])
	@mock.patch('demo_utils.ambari.Ambari.get_service', return_value=[json.loads(service_info_started), ''])
	@mock.patch('demo_utils.curl_client.CurlClient.make_request', return_value=[change_response_same, ''])
	...
	@mock.patch('demo_utils.shell.Shell.run', side_effect=[[service_info_stopped, ''], [service_info_stopped, ''], [service_info_stopped, '']])
	@mock.patch('demo_utils.ambari.Ambari.get_service', return_value=[json.loads(service_info_stopped), ''])
	@mock.patch('demo_utils.curl_client.CurlClient.make_request', return_value=[change_response_same, ''])
	@mock.patch('time.sleep', return_value=1)
	...
	@mock.patch('demo_utils.shell.Shell.run', side_effect=[[service_info_stopped, ''], [service_info_stopped, ''], [service_info_stopped, '']])
	@mock.patch('demo_utils.ambari.Ambari.get_service', return_value=[json.loads(service_info_stopped), ''])
	@mock.patch('demo_utils.curl_client.CurlClient.make_request', return_value=[bad_res_diff, ''])
	@mock.patch('time.sleep', return_value=1)
