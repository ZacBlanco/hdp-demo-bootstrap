...
class TestZeppelinAddNotebook(unittest.TestCase):
  ...
  @mock.patch('demo_utils.curl_client.CurlClient.make_request', side_effect=[['200 OK', ], ['500 err', ''], ['201 Created', ''], ['201 Created', '']])
  @mock.patch('os.path.isfile', return_value=True)
  @mock.patch('os.listdir', return_value=['note1.json', 'note2.json', 'note3.xml'])
  ...
  def test_post_notebook(self, mock1, mock2):
    ...
    assert not service_installer.post_notebook(paths[0], 'JSESSIONID=null')
    
    assert service_installer.post_notebook(paths[1], 'JSESSIONID=null')
    
    assert service_installer.post_notebook(paths[2], 'JSESSIONID=null')
  ...
  @mock.patch('demo_utils.curl_client.CurlClient.make_request', return_value=['''HTTP/1.1 200 OK
Date: Wed, 17 Aug 2016 11:13:15 GMT
Access-Control-Allow-Origin:
Access-Control-Allow-Credentials: true
Access-Control-Allow-Headers: authorization,Content-Type
Access-Control-Allow-Methods: POST, GET, OPTIONS, PUT, HEAD, DELETE
Date: Wednesday, August 17, 2016 11:13:15 AM UTC
Set-Cookie: rememberMe=deleteMe; Path=/; Max-Age=0; Expires=Tue, 16-Aug-2016 11:13:15 GMT
Set-Cookie: JSESSIONID=f1c506f9-a1a4-466a-859e-b13cb0b22ecc; Path=/; HttpOnly
Set-Cookie: rememberMe=deleteMe; Path=/; Max-Age=0; Expires=Tue, 16-Aug-2016 11:13:15 GMT
...
{"status":"OK","message":"","body":{"principal":"admin","ticket":"47a1fe3a-593d-47ce-85bb-f6e7238c6dcb","roles":"[]"}}''', ''])
  def test_get_session(self, mock1):
    res = service_installer.get_zeppelin_session('admin', 'password1')
    print res
    assert 'JSESSIONID' in res
    assert '=' in res
