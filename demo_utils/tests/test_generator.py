...
#      print("KEY: " + key + " | OCC: " + str(occ[key]) + " | PROB: " + str( occ[key]/float(test_num) ))
  ...
  def test_json_string_config(self):
    data = """[{"fieldName": "test_field", "type":"string", "values":["a", "b", "c"]}]"""
    vals = []
    try:
      gen = DataGenerator(data)
      for i in range(50):
        data = gen.generate()
        if not data['test_field'] in vals:
          vals.append(data['test_field'])
    except ValueError as e:
      self.fail("Shouldn't have failed when generating data")
    assert len(vals) == 3, 'Should have at least 3 different values generated'
