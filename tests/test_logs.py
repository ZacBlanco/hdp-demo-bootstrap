...
class TestLogs(unittest.TestCase):
	...
	@mock.patch('scripts.config.read_config', return_value={'LOGGING': {'log-level': 'DEBUG'}})
	def test_debug(self, mock1):
		logger = Logger('debug').getLogger()
		handler = logger.handlers[0]
		assert handler.level == 10
		
	@mock.patch('scripts.config.read_config', return_value={'LOGGING': {'log-level': 'INFO'}})
	...
	@mock.patch('scripts.config.read_config', return_value={'LOGGING': {'log-level': 'WARN'}})
	def test_warn(self, mock1):
		logger = Logger('warning').getLogger()
		handler = logger.handlers[0]
		assert handler.level == 30
	
	@mock.patch('scripts.config.read_config', return_value={'LOGGING': {'log-level': 'ERROR'}})
	def test_error(self, mock1):
		logger = Logger('error').getLogger()
		handler = logger.handlers[0]
		assert handler.level == 40
	
	@mock.patch('scripts.config.read_config', return_value={'LOGGING': {'log-level': 'CRITICAL'}})
	...
	@mock.patch('scripts.config.read_config', return_value={'LOGGING': {'bad-level': 'MISSING'}})
	def test_bad(self, mock1):
