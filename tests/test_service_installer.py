...
from package.util import service_installer
...
class TestHDPSelectInstall(unittest.TestCase):
	...
	@mock.patch('package.util.shell.Shell.run', return_value=['/usr/bin/hdp-select', ''])
	...
	@mock.patch('platform.linux_distribution', return_value=centos_6_distro)
	@mock.patch('package.util.shell.Shell.run', return_value=['', ''])
...
class TestComponentCheck(unittest.TestCase):
	...
	@mock.patch('package.util.shell.Shell.run', return_value=['/usr/bin/ambari-server', ''])
...
class TestZeppelinInstall(unittest.TestCase):
	...
	@mock.patch('package.util.service_installer.is_ambari_installed', return_value=False)
	...
	@mock.patch('package.util.service_installer.is_ambari_installed', return_value=True)
	@mock.patch('package.util.service_installer.is_hdp_select_installed', return_value=False)
	@mock.patch('package.util.service_installer.install_hdp_select', return_value=False)
	...
	@mock.patch('package.util.service_installer.is_ambari_installed', return_value=True)
	@mock.patch('package.util.service_installer.is_hdp_select_installed', return_value=True)
	@mock.patch('package.util.service_installer.install_hdp_select', return_value=True)
	@mock.patch('package.util.service_installer.check_ambari_service_installed', return_value=True)
	...
	@mock.patch('package.util.service_installer.is_hdp_select_installed', return_value=True)
	@mock.patch('package.util.service_installer.install_hdp_select', return_value=True)
	@mock.patch('package.util.service_installer.check_ambari_service_installed', return_value=False)
...
class TestAmbariServiceCheck(unittest.TestCase):
	...
	@mock.patch('package.util.curl_client.CurlClient.make_request', side_effect=[['', ''], ['200 OK', '']])
	@mock.patch('__builtin__.raw_input', side_effect=['\n', '\n', 'v', 'n'])
	...
	@mock.patch('package.util.curl_client.CurlClient.make_request', side_effect=[['200 OK', ''], ['200 OK', '']])
	@mock.patch('__builtin__.raw_input', side_effect=['\n', '\n', 'v', 'n'])
	...
	@mock.patch('package.util.curl_client.CurlClient.make_request', side_effect=[['', ''],['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']])
	@mock.patch('__builtin__.raw_input', side_effect=['', '', '', '', '', '', '', '', '', '', ''])
...
class TestZeppelinAddNotebook(unittest.TestCase):
	...
	@mock.patch('package.util.curl_client.CurlClient.make_request', return_value=['201 Created', ''])
	...
	@mock.patch('package.util.curl_client.CurlClient.make_request', return_value=['500 err', ''])
	@mock.patch('os.path.isfile', return_value=True)
	@mock.patch('os.listdir', return_value=['note1.json', 'note2.json', 'note3.json'])
	...
	@mock.patch('package.util.curl_client.CurlClient.make_request', side_effect=[['500 err', ''], ['201 Created', ''], ['201 Created', '']])
	@mock.patch('os.path.isfile', return_value=True)
	@mock.patch('os.listdir', return_value=['note1.json', 'note2.json', 'note3.xml'])
